...
from src.loss_function import BaseLossFunction, LossFunction
...
class BaseMatrixFactorization(ABC, torch.nn.Module):
    ...
    def __init__(
        ...
        self.embedding_table = torch.nn.Embedding(
            number_of_users + number_of_items, embedding_dim
        )
    ...
    def get_item_embedding(self, item_ids: torch.Tensor) -> torch.Tensor:
        ...
        return self.embedding_table(item_ids)
...
class LightGCN(BaseMatrixFactorization):
    ...
    def __init__(
        ...
        embedding_dim: int,
        num_layers: int,
        loss_function: BaseLossFunction = LossFunction["BPR"].value(),
        **kwargs,
    ) -> None:
    ...
    def forward(
        ...
        positive_item_embedding = (
            total_embedding[positive_item_ids] if positive_item_ids != None else None
        )
        negative_item_embedding = (
            total_embedding[negative_item_ids] if negative_item_ids != None else None
        )
    ...
    def get_all_embbedings(self, graph=dgl.DGLGraph, **kwargs) -> torch.Tensor:
...
class TAGCF(LightGCN):
    ...
    @torch.no_grad()
    def get_all_embbedings(
        self, graph: dgl.DGLGraph, use_mp: bool, **kwargs
    ) -> torch.Tensor:
