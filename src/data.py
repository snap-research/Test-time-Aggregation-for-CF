...
class ML1MDataset(BaseDataset):
        ...
        def __init__(self, 
            ...
            self.train_indices = indices[:len(indices) * train_ratio]
            self.valid_indices = indices[len(indices) * train_ratio:
                                         len(indices)* (train_ratio+valid_ratio)]
            self.test_indices = indices[len(indices)* (train_ratio+valid_ratio):]
